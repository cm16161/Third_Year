...
public class RingTreblecross {
    ...
    **/
         ...
         */
         ...
         for(int count =0; count<board.length;count++){
           ...
           if(board[count] == true){
              consecutive++;
             for(int j=1; j<k;j++){
               if (board[((count+j)%(board.length))] == true){
                 consecutive++;
               }
             }
           }
    ...
    public Best minValueAndBestSuccessor(GameState state) {
         ...
         */
         int localMin = 2;
         GameState localBest = state;
         if (isTerminalState(state)){
            return new Best(1, null);
         }
         else{
           for (GameState s : getSuccessors(state)){
             if(localMin > maxValueAndBestSuccessor(s).score){
               localMin = maxValueAndBestSuccessor(s).score;
               localBest = s;
             }
           }
         }
     return new Best(localMin, localBest);
    }
         ...
         */
         ...
         int localMax = -2;
          if (isTerminalState(state)){
            ...
            for (GameState s : getSuccessors(state)){
              if(localMax < minValueAndBestSuccessor(s).score){
                localMax = minValueAndBestSuccessor(s).score;
                localBest = s;
              }
            }
          }

    	  return new Best(localMax, localBest);
    }
         ...
         */
         ...
         while (count < board.length){
           if(board[count]==true){
             ...
             boolean[] tempBoard = state.getBoard();
             tempBoard[count] = true;
             GameState newState = new GameState(tempBoard);
             states.add(newState);
             count++;
           }
         }
