...
int main(){
	...
	float C1[26] = {0,0,3,0,2,1,5,1,0,2,4,0,0,1,1,3,2,1,0,6,3,4,1,0,1,0};
	float C2[26] = {5,0,0,0,1,0,0,3,0,3,2,2,0,1,2,4,0,0,5,0,4,3,1,0,4,1};
	float C3[26] = {4,7,0,0,1,0,0,0,2,1,3,3,6,0,1,2,3,0,1,1,0,1,4,0,0,0};
	float C4[26] = {2,0,1,0,0,0,0,3,2,0,2,5,0,1,4,2,0,0,3,2,2,5,0,0,2,4};
	float C5[26] = {0,0,5,0,0,2,0,2,0,2,0,2,4,5,2,1,2,0,0,0,7,0,0,1,5,0};
	float C6[26] = {3,1,1,1,4,1,2,1,5,0,2,1,1,3,2,0,0,4,0,4,0,2,1,0,1,0};
	float C7[26] = {5,1,1,0,0,0,0,3,0,1,2,9,2,1,2,2,0,0,1,2,1,2,1,0,1,3};
	float C8[26] = {1,2,0,0,0,3,1,0,4,1,0,4,1,1,1,2,0,0,0,0,4,0,4,2,7,2};
	float C9[26] = {4,1,1,1,4,0,0,2,2,0,1,1,1,5,7,1,0,2,1,4,0,1,1,0,0,0};
	...
	for (int k=0;k<26;k++){
		for(int i=0;i<26;i++){
			...
			totalProbability[k]+= (C3[i] * Q[((i+k)%26)]);
	}
